...
type MPRepository interface {
	...
	CreatePreference(ctx context.Context, orderID uuid.UUID, amount int64, expiresAt time.Time, items []domain.OrderItem, payer dto.PayerInfo) (preferenceID string, checkoutURL string, err error)
